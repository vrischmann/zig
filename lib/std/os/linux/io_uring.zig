...
pub const IO_Uring = struct {
    ...
    pub fn recv(
        ...
        switch (buffer) {
            ...
            .iovecs => |vecs| {
                if (vecs.len <= 0) return error.BufferInvalid;
                var iovec_buffer = vecs[0].iov_base[0..vecs[0].iov_len];
                io_uring_prep_recv(sqe, fd, iovec_buffer, 0);
            },
            .buffer_selection => |selection| {
...
const SocketTestHarness = struct {
    ...
    fn close(self: SocketTestHarness) void {
        os.close(self.client);
        os.close(self.listener);
    }
};
